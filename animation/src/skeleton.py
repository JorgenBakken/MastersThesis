...
class Skeleton(object):
    ...
    def precompute_local_matrices(self):
        ...
        for key, bone in self.bones.items():
            C = np.dot(Rz(bone.axis[2]), np.dot(Ry(bone.axis[1]), Rx(bone.axis[0])))
            Cinv = C.transpose()
    ...
    def setup_skeleton_neutral_coords(self):
        ...
        get_child_coords('root', np.zeros(3))
        self._coords = coords
    ...
    def get_coords_for_frame(self, frame, root_offset = None):
        ...
        R = np.dot(Rz(pl.deg2rad(transf[5])), np.dot(Ry(pl.deg2rad(transf[4])), Rx(pl.deg2rad(transf[3]))))
        B = T(transf[0:3])
        rpos = np.dot(B, [0,0,0,1])/0.45 * Skeleton.SCALE
